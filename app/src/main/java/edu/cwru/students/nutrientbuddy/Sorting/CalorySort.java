...
public class CalorySort implements SearchMetric {
    ...
    public ArrayList<Food> sort(ArrayList<Food> list) {
        Comparator<Food> comp = new Comparator<Food>() {
            ...
            public int compare(Food food1, Food food2) {
                float first = Float.parseFloat(food1.get(Food.totalCalories)+"");
                float second = Float.parseFloat(food2.get(Food.totalCalories)+"");
                if(first==second){
                    return 0;
                }else{
                    if(first < second){
